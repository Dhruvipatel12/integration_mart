...
def indiamart_call():
    ...
    for x2 in x['RESPONSE']:
        ...
        for i in list:
            ...
            data= frappe.db.sql("select sender_id from `tabIndiamart Data` where sender_id=%s",i["UNIQUE_QUERY_ID"])
            dt=""
            for i1 in data:
                for j1 in i1:
                    dt=j1
            if dt != i["UNIQUE_QUERY_ID"]:
                a= frappe.get_doc(({"doctype" : "Indiamart Data",  "sender_id": i['UNIQUE_QUERY_ID'], "full_name": i['SENDER_NAME'], 	"mobile": i['SENDER_MOBILE'], "mobile_2": i['SENDER_MOBILE_ALT']
